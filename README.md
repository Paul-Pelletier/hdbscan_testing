...
2. **2D Path Construction**
   - For each surface and each moneyness slice, construct a 2D path: $\gamma(s) = (\log(\text{maturity}), \text{IV})$.
   - This encodes the smile geometry as a path in $\mathbb{R}^2$.

3. **Chen–Lyons Path Signature Encoding**
   - For each 2D path, compute the truncated signature $S^{(d)}(\gamma)$ up to depth $d$ using the `signatory` library.
   - The signature is a collection of iterated integrals:
     $$
     S^{(d)}(\gamma) = \left(1, \int d\gamma, \int d\gamma \otimes d\gamma, \ldots, \int \cdots \int d\gamma \right)
     $$
...
- $\rho$ = skewness (controls asymmetry)
- $m$ = horizontal shift
- $\sigma$ = curvature (controls smile convexity)

Regimes are defined by maturity-dependent schedules for $(a, b, \rho, m, \sigma)$, e.g.:
...
### Path Signature
Given a path $\gamma: [0,1] \to \mathbb{R}^2$, the signature up to depth $d$ is:
$$
S^{(d)}(\gamma) = \left(1, S^1, S^2, \ldots, S^d\right)
$$
where $S^k$ is the $k$-fold iterated integral:
$$
S^k = \int_{0 < t_1 < \cdots < t_k < 1} d\gamma_{t_1} \otimes \cdots \otimes d\gamma_{t_k}
$$
...
Given a sequence of predicted regimes $r_t$ and probabilities $p_t$, the sticky regime $\hat{r}_t$ is defined recursively:
- At each $t$, consider the window $[t-W+1, t]$
- If a regime appears in the majority and $\text{mean}(p_t)$ exceeds a threshold, accept the change
